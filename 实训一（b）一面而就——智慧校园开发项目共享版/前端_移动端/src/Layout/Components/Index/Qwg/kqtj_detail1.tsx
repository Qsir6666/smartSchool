...
const App: React.FC = () => {
  ...
  // console.log(selectDate, '1331133');
  ...
  // const grades = ['全部年级', '一年级', '二年级', '三年级', '四年级', '五年级', '六年级'];
  const grades = ['全部学院', '人工智能学院', '云计算学院', '大数据学院', '数智传媒学院', '鸿蒙生态开发学院', '元宇宙学院'];
  const [classData, setClassData] = useState([])
  // const classData = [
  //   { name: '一年级 1 班', total: 45, present: 43, cate2: 1, early: 0, leave: 1 },
  //   {
  //     "name": "二年级 1 班",
  //     "total": 20,   //总数
  //     "present": 12,   //实到人数
  ...
  const totalStudents = classData.reduce((acc, curr) => acc + curr.total, 0) //学生总人数
  const totalPresent = classData.reduce((acc, curr) => acc + curr.present, 0) //实到人数
  const attendanceRate = totalStudents > 0
    ? ((totalPresent / totalStudents) * 100).toFixed(1)
    : '0.0'  //出勤率
  // console.log(totalStudents,totalPresent,attendanceRate);

  const getMsgByDate = () => {
    const localDateStr = dayjs(selectDate).format('YYYY-MM-DD')
    axios.post('http://localhost:3000/QWG/getMsgByDate', {
      date: localDateStr
    }).then(res => {
    ...
    Dialog.confirm({
      ...
      onConfirm: async () => {
        ...
        const data = [
          ...
          // 增加总统计行
          ['全校汇总',
            classData.reduce((sum, d) => sum + d.total, 0),
            classData.reduce((sum, d) => sum + d.present, 0),
            `${((classData.reduce((s, d) => s + d.present, 0) /
              classData.reduce((s, d) => s + d.total, 0)) * 100).toFixed(1)}%`,
            classData.reduce((s, d) => s + d.cate2, 0),
            classData.reduce((s, d) => s + d.cate3, 0),
            classData.reduce((s, d) => s + d.cate4, 0)
          ]
        ];
        // 创建工作簿
  ...
  // 点击班级，进入班级考勤数据详情页
  const gokqtj_1_child = (classInfo) => {
    navigate('/kqtj_1_child', { state: { classInfo: classInfo, dateStr: dateStr } })
  }
    ...
    if (viewMode === 'chart') {
      ...
      if (chartDom) {
        ...
        const option = {
          ...
          series: [{
            data: classData.map(c => ((c.present / c.total) * 100).toFixed(1)),
            type: 'bar',
            barWidth: '40%',
            itemStyle: {
              color: '#4F46E5'
              ...
              {viewMode === 'list' ? (
                <div className="space-y-4">
                  {classData
                    ...
                    .map((classInfo, index) => (
                      <div key={index} className="bg-white rounded-lg p-4 shadow-sm" onClick={() => { gokqtj_1_child(classInfo) }}>
